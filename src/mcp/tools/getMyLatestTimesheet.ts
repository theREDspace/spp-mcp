...
const getMyLatestTimesheet: Tool = {
  ...
  async handler(_args, _ctx) {
    ...
    try {
      const resolvedUser = await resolveMe(client);
      ...
      if (latest.approved_by) {
        try {
          const approver = await client.read('User', latest.approved_by) as any;
      ...
      return errorResponse(err, 'fetching your latest timesheet');
    }
  },
};
